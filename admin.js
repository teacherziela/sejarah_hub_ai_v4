...
const moduleMap={
  ...
  pengumuman:{title:'Pengumuman', fields:[['Tahun','tahun','number',true,CONFIG.DEFAULT_YEAR],['Tajuk','tajuk','text',true],['Tarikh','tarikh','date',true],['Isi','isi','textarea'],['Link','link'],['Status','status','text',false,'Aktif']]},
  ...
  galeri:{title:'Galeri', fields:[['Tahun','tahun','number',true,CONFIG.DEFAULT_YEAR],['Tajuk','tajuk','text',true],['Tarikh','tarikh','date',true],['Kelas','kelas'],['Penerangan','penerangan','textarea'],['Photo','photo'],['Status','status','text',false,'Aktif']]},
...
function toDateInputValue(v){
  ...
  const my=s.match(/^(\d{1,2})[\/\-.](\d{1,2})[\/\-.](\d{4})$/);
  if(my) return `${my[3]}-${String(my[2]).padStart(2,'0')}-${String(my[1]).padStart(2,'0')}`;
  const d=new Date(s);
  if(isNaN(d)) return '';
  const pad=n=>String(n).padStart(2,'0');
  return `${d.getFullYear()}-${pad(d.getMonth()+1)}-${pad(d.getDate())}`;
...
function editRecord(id){ const r=getList().find(x=>String(idOf(x))===String(id)); if(!r)return; const m=moduleMap[current]; editId=id; $('formTitle').textContent='✏️ Edit '+m.title; $('recordId').value=id; m.fields.forEach(([label,key,type='text'])=>{ if($(key)){ const raw=r[label] || r[key] || ''; $(key).value = type==='date' ? toDateInputValue(raw) : raw; } }); scrollTo({top:0,behavior:'smooth'}); }
