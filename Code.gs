...
function examItemDefs_(tingkatan){
  var out=[];
  var i;
  ...
  var t=String(tingkatan||'1');
  var scheme=t==='2'
    ? { E1:[4,8,8], E2:[6,6,8] }
    : { E1:[6,6,8], E2:[4,8,8] };

  ['E1','E2'].forEach(function(key){
    var marks=scheme[key];
...
function readItemMap(tingkatan, ujian){
  ...
  return { success:true, rows:rows, items:examItemDefs_(tingkatan) };
}
...
function saveExamRecord(data){
  try{
    ...
    var defs=examItemDefs_(data.tingkatan);
...
function saveItemMapBatch(data){
  try{
    ...
    var defs=examItemDefs_(tingkatan);
    var defByKey={};
