...
// Tiada DriveApp. Rumusan dikira ikut kelas: 1 murid = 1 TP terkini.
...
function readModule(module, year){
  ...
  var values = obj.sh.getDataRange().getValues();
...
function valueForHeader(h,d){
  var map = {'Nama':'nama','Kad Pengenalan':'kadPengenalan','Jawatan':'jawatan','Opsyen':'opsyen','Ijazah':'ijazah','Pengalaman':'pengalaman','Kelas':'kelas','Email':'email','Telefon':'telefon','Foto':'foto','Status':'status','Tahun':'tahun','Tajuk':'tajuk','Tarikh':'tarikh','Isi':'isi','Link':'link','Tingkatan':'tingkatan','Kategori':'kategori','Icon':'icon','Penerangan':'penerangan','Photo':'photo','Bab':'bab','Jenis':'jenis'};
  return d[map[h]] || d[h] || '';
}

function normalize_(s){ return String(s || '').toLowerCase().replace(/\s+/g,'').replace(/_/g,'').trim(); }
function rowHasData_(row){ for(var i=0;i<row.length;i++){ if(row[i] !== '' && row[i] !== null) return true; } return false; }
...
function cleanName_(s){ return String(s||'').replace(/\s*KPM-Guru\s*$/i,'').trim(); }
function getDateNum_(v){ var d = new Date(v); return isNaN(d.getTime()) ? 0 : d.getTime(); }
function fmtDate_(v){ var d = new Date(v); if(isNaN(d.getTime())) return String(v||''); return Utilities.formatDate(d, Session.getScriptTimeZone(), 'dd/MM/yyyy'); }
function tpNum_(v){ var raw=String(v||'').replace(/[^\d.]/g,''); return Number(raw||0); }
...
function rekodObj_(o){
  ...
  return {
    ...
    idMurid:String(pick_(o,['IDMurid','ID Murid'])).trim(),
    nama:String(pick_(o,['Nama Murid','Nama'])).trim(),
    tingkatan:ting,
    kelas:kelas,
...
function readPbdInitLite(){
  ...
  return {
    success:true,
    murid: muridSh ? rowsToObjects_(muridSh.getDataRange().getValues()).map(muridObj_) : [],
    topik: topikSh ? rowsToObjects_(topikSh.getDataRange().getValues()) : [],
    rekod: []
  };
}
...
function readPbdClassSummary(tingkatan, kelas){
  try{
    ...
    var muridAll=rowsToObjects_(muridSh.getDataRange().getValues()).map(muridObj_);
    var murid=muridAll.filter(function(m){
      var st=String(m.status||'AKTIF').toUpperCase();
      return st !== 'PINDAH' && st !== 'TIDAK AKTIF' && String(m.tingkatan)===String(tingkatan) && String(m.kelas)===String(kelas);
    });

    var rekodAll=rowsToObjects_(rekodSh.getDataRange().getValues()).map(rekodObj_);
    ...
    for(var r=0;r<rekodAll.length;r++){
      ...
      if(!latestByKey[key] || rec.dateNum > latestByKey[key].dateNum || (rec.dateNum === latestByKey[key].dateNum && rec.row > latestByKey[key].row)){
        latestByKey[key]=rec;
      }
    }
...
function findGuruForClass_(guruSh, tingkatan, kelas, rekodAll){
  ...
  var names=[];
  if(guruSh){
    var gurus=rowsToObjects_(guruSh.getDataRange().getValues());
...
function unique_(arr){ var seen={}, out=[]; for(var i=0;i<arr.length;i++){ if(!seen[arr[i]]){ seen[arr[i]]=true; out.push(arr[i]); } } return out; }
...
function testPbdClassSummary(){
  var data=readPbdClassSummary('1','ADIL');
